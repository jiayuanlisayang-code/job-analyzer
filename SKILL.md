...
**核心原则：岗位筛选必须基于用户真实简历，不能基于用户表达的"理想方向"。**

当用户说要往某个行业转型（如"电网/算力供应链"），但简历上完全没有相关经验时：
- 不要筛选该行业岗位——简历筛不过
- 找 **"桥接层"**：当前行业 + 目标行业交叉的公司
- 例如用户有物流供应链经验、想做电力行业 → 推荐华能智链（华能集团旗下供应链公司）、阿里云能源线、中数联物流科技等"做物流供应链 + 服务电力客户"的公司
- 用户先进这类公司 → 做项目积累行业知识 → 1-2 年后自然转目标行业

输入增强：主动询问是否有简历文件，阅读后提取核心经验关键词用于搜索。
