...
> mcp-jobs 关键词匹配仅覆盖"JD 中出现了搜索词"的岗位。物流/供应链行业的公司名（如"中数联""申通""满帮""壹米滴答"）本身不含"AI""产品经理"等热词，在嘈杂的 42 条结果中极易被淹没。因此：
> 1. **公司名直搜为强制步骤**——不依赖关键词召回，用公司名直接搜 `mcp_search_job`
> 2. **行业清单覆盖检查**——搜索结束后，对照行业公司清单逐一确认是否命中，未命中则 WebSearch 补搜
> 3. **外企官网直搜**——mcp-jobs 不爬外企官网（如 flexport.com/careers），需单独 WebSearch
...
**当用户目标行业匹配以上任一领域时，必须对公司名列表中每个公司做直搜+WebSearch补查。**
...
3. **🚨 公司名直搜（强制步骤，弥补关键词召回盲区）：**
   → 对照上方行业公司清单，对目标行业对应的每家中国公司，用 `mcp_search_job` 搜 `keyword: "[公司名] 产品经理"` + `location: [地点]`
   → 每轮并行 3-5 家公司，不要串行（耗时）
4. 对关键岗位调用 `mcp_job_detail` 获取完整 JD 描述和更多信息
5. **过滤：删除所有 `url` 为空或无效的岗位**
6. **🚨 覆盖检查（强制步骤）：**
   → 将搜索命中的公司列表与行业公司清单对照
   → 未命中的公司 → 用 WebSearch 搜 `"[公司名] [目标岗位关键词] 招聘 [地点] [年份]"` 补查
   → 确认外企板块可用 WebSearch（`site:careers.dhl.com` 等）
   → 补查结果中提取 URL 并入岗位列表
7. **保留 ≥15 个有效岗位供筛选**（若公司名直搜命中多，此数应轻松超过）
