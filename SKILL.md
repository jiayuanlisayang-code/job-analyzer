...
**🚨 数据源优先级：优先尝试 mcp-jobs（结构化数据更优），但安装前必须满足以下条件：**

1. 当前环境允许安装 npm 依赖（非企业限制/沙箱/CI 环境）
2. 用户未明确拒绝安装
3. 已备份现有 MCP 配置
4. 不覆盖已验证可用的 node.exe 直启配置

**若任一条件不满足，直接进入 WebSearch + 官网回退路径，并在报告中标注"数据源降级"。**
...
1. ❌ 不检测 mcp-jobs 可用性就直接走 WebSearch（应先检测后判断）
...
从所有 JD 中提炼高频能力要求。**维度不固定——根据目标岗位和 JD 高频词动态生成 5-7 个维度。**

默认候选维度池：

1. **岗位专业能力**：该岗位的核心专业技能（如 PM 的 PRD/路线图；销售的客户开发/谈判；工程师的系统设计/架构）
2. **工具/技术能力**：JD 中高频出现的工具、语言、框架（如 SQL/Python/Axure/LangChain 等）
3. **业务/行业理解**：根据用户目标行业定制，提取 JD 中的业务关键词（物流→TMS/WMS/路由；金融→风控/量化/合规）
4. **数据与分析能力**：数据驱动决策、指标体系、A/B 实验、报表（多数岗位通用）
5. **项目/流程/协作能力**：项目管理、跨部门推进、敏捷/DevOps（通用）
6. **沟通与影响力**：双语能力、利益相关方管理、汇报/演示（通用）
7. **合规/风险/安全**：仅当 JD 中高频出现（如金融合规、AI 安全护栏、数据隐私）时才启用

**动态生成规则：**
- AI 相关岗位 → 可在「工具/技术能力」中细化 LLM/RAG/Agent/Prompt Engineering
- 非 AI 岗位（销售/运营/HR/工程师等）→ 不得强制输出 AI/ML 维度
- 每个维度列出 4-5 个知识点，标注来源于哪些公司的 JD
- 维度名和知识点全部从真实 JD 中提取，不做预设
...
| 阶段四 | ✅/❌ | 5-7 维度动态能力模型已提炼 | 每维度 ≥4 知识点，维度从 JD 提取不预设 |
